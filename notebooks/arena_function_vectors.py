...
if MAIN:
    model = LanguageModel(
        "EleutherAI/gpt-j-6b", device_map="auto", torch_dtype=t.bfloat16
    )
...
if MAIN:
    ...
    # Solution to attention pattern plot
...
if MAIN:
    ...
    correct_logprobs_zero_shot, correct_logprobs_intervention = (
        calculate_h_and_intervene_logprobs(
            model, dataset, zero_shot_dataset, layer=layer
        )
    )
...
if MAIN:
    ...
    for layers in batch_process_layers(N_LAYERS, batch_size=4):
        ...
        results = t.concat(
            [
                results,
                calculate_fn_vectors_and_intervene(model, dataset, layers).to(device),
            ]
        )
...
if MAIN:
    ...
    with open(
        section_dir / "data" / "country_capital_pairs.txt", "r", encoding="utf-8"
    ) as f:
    ...
    _COUNTRY_CAPITAL_PAIRS = [
        pair for pair in COUNTRY_CAPITAL_PAIRS if pair[0] != country
    ]

    # Define our dataset, and the attention heads we'll use
    dataset = ICLDataset(
        _COUNTRY_CAPITAL_PAIRS, size=20, n_prepended=5, bidirectional=False
    )
    head_list = [
        (8, 0),
        (8, 1),
